...
void BL_on_AD57_Read_SD_Push_CAN_2_Flash_Remote_Appl ( uint16_t p_id, uint16_t  dir_list_index )
{
  #if ( defined BUILD_AD57_BL )
  {
    ...
    //  from the heartbeat that was flowing before the trigger; if we don't clear it,
    //  the "wait for established" guard below is skipped for chunk 0 and we start
    //  bursting MDP data before the remote side is listening again.
    //
